...
class TeamManager {
    ...
    public function add(Team $tm) {
        $q = $this->_bdd->prepare('INSERT INTO elo_teams SET name = ":name", played = :played, rating = :rating, oldrating = :oldrating, wins = :wins, losses = :losses, active = :active, logo = ":logo"');
        ...
        $q->bindValue(':active', $tm->getActive(), PDO::PARAM_INT);
        $q->bindValue(':logo', $tm->getLogo());
        $q->execute();
    }
    ...
    public function getFromName($name) {
        $q = $this->_bdd->query('SELECT * FROM elo_teams WHERE name = "' . $name.'"');
        if ($data = $q->fetch())
            return  new Team($data);
        return null;
    }
    ...
    public function getAll() {
      $teams = [];
      $q = $this->_bdd->query('SELECT * FROM elo_teams');
    ...
    public function update(Team $tm) {
      $q = $this->_bdd->prepare('UPDATE elo_teams SET name = :name, played = :played, rating = :rating, oldrating = :oldrating, wins = :wins, losses = :losses, active = :active WHERE id = :id');
